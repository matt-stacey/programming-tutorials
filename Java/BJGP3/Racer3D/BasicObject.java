// Racer3D
// Matt Stacey
// 29 Nov 2012

package Racer3D;

import java.awt.*;

// BasicObject class for rectangular/polygonal shapes and hitboxes

public class BasicObject {
	
	// variables
	private Shape shape;
	private boolean alive;
	private double x, y, z; // forward, left, up
	private double velX, velY, velZ;
	private double moveAngle, faceAngle; // no Z directions
	private double rollAngle; // for on walls
	private Color color;
	
	// accessors
	public Shape getShape() { return shape; }
	public boolean isAlive() { return alive; }
	public double getX() { return x; }
	public double getY() { return y; }
	public double getZ() { return z; }
	public double getVelX() { return velX; }
	public double getVelY() { return velY; }
	public double getVelZ() { return velZ; }
	public double getFaceAngle() { return faceAngle; }
	public double getMoveAngle() { return moveAngle; }
	public double getRollAngle() { return rollAngle; }
	public Color getColor() { return color; }
	
	// mutators/helpers
	public void setShape(Shape shape) { this.shape = shape; }
	public void setAlive(boolean alive) { this.alive = alive; }
	public void setX(double x) { this.x = x; }
	public void incX(double i) { this.x += i; }
	public void setY(double y) { this.y = y; }
	public void incY(double i) { this.y += i; }
	public void setZ(double z) { this.z = z; }
	public void incZ(double i) { this.z += i; }
	public void setVelX(double velX) { this.velX = velX; }
	public void incVelX(double i) { this.velX += i; }
	public void setVelY(double velY) { this.velY = velY; }
	public void incVelY(double i) { this.velY += i; }
	public void setVelZ(double velZ) { this.velZ = velZ; }
	public void incVelz(double i) { this.velZ += i; }
	public void setFaceAngle(double angle) { this.faceAngle = angle; }
	public void incFaceAngle(double i) { this.faceAngle += i; }
	public void setMoveAngle(double angle) { this.moveAngle = angle; }
	public void incMoveAngle(double i) { this.moveAngle += i; }
	public void setRollAngle(double angle) { this.rollAngle = angle; }
	public void incRollAngle(double i) { this.rollAngle += i; }
	public void setColor(Color color) { this.color = color; }

	public BasicObject() {
		setShape(null);
		setAlive(false);
		setX(0.0);
		setY(0.0);
		setZ(0.0);
		setVelX(0.0);
		setVelY(0.0);
		setVelZ(0.0);
		setFaceAngle(0.0);
		setMoveAngle(0.0);
		setRollAngle(0.0);
		setColor(null);
	}
}